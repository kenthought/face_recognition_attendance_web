  ...
  methods: {
    ...
    handleSubmit() {
      ...
      if (this.editing != true) {
        const db = getDatabase();
        const teacherKey = push(child(ref(db), "teachers")).key;
        set(ref(db, "teachers/" + teacherKey), {
          id: teacherKey,
        ...
        const db = getDatabase();
        const updates = {};
        updates["/teachers/" + this.item.id] = this.item;
    ...
    deleteRecord() {
      const db = getDatabase();
      remove(ref(db, "/teachers/" + this.item.id))
        .then((response) => {
          this.$nextTick(() => {
            this.$bvModal.hide("modal_add");
          });
